...
class AuditReport(FPDF):
    def header(self):
        ...
        self.cell(0, 10, f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", border=False, ln=True, align="R")
        self.ln(10)
...
def generate_prediction_pdf(result, input_data, goods_code, goods_description):
    """
    ...
    if feature_importance:
        pdf.ln(5)
        pdf.set_font("helvetica", "B", 14)
        pdf.cell(0, 10, "3. Feature Contributions (Explainability)", ln=True)
        pdf.ln(2)
        pdf.set_font("helvetica", "I", 9)
        pdf.multi_cell(0, 5, "This table shows the contribution of each feature to the final prediction. "
                            "Positive values increased the predicted price, while negative values decreased it.")
        pdf.ln(2)

        pdf.set_font("helvetica", "B", 10)
        pdf.cell(100, 8, "Feature Name", 1, 0, "C", fill=True)
        pdf.cell(90, 8, "Impact (Price Units)", 1, 1, "C", fill=True)
        
        pdf.set_font("helvetica", "", 9)
        # Sort by absolute impact
        sorted_fi = sorted(feature_importance.items(), key=lambda x: abs(x[1]), reverse=True)
        for feat, imp in sorted_fi:
            pdf.cell(100, 7, feat, 1)
            # Add color indicator if possible or just use +-
            color_str = "+" if imp > 0 else ""
            pdf.cell(90, 7, f"{color_str}{imp:.4f}", 1, 1, "R")
